...
const profilePosts = async () => {
  ...
  const response = await axios.get(
    `http://localhost:8000/api/posts?userId[eq]=${userId.value}`,
    { headers: header }
  );
...
onMounted(async () => {
  ...
  followStatus.value = user.value.isFollowed;
  await getAuthUser();
  await profilePosts();
  loadingStatus.value = false;
});

watch(followStatus, async () => {
  await userStore.showUser(route.params.userId);
  user.value = userStore.user;
});
</script>
<template>
  ...
  <div
    class="flex flex-col p-4 border-b-[1px] border-b-slate-900/5 dark:border-b-gray-600"
  >
    <h3 class="text-lg font-bold dark:text-gray-300">{{ user.name }}</h3>
    <span class="dark:text-gray-300">@{{ user.nickName }}</span>
    <span class="dark:text-gray-300"
      ><h3 class="font-bold inline-block">{{ user.followers }}</h3>
      followers</span
    >
  </div>
  <div
  ...
  >
    ...
    <div class="flex flex-col w-full gap-1 items-start">
      ...
      <div
        ...
        class="grid grid-cols-2 w-full gap-2 rounded-2xl overflow-hidden"
      >
        <div
