...
const gridWrapper = ref("relative grid w-full gap-4 rounded-2xl overflow-hidden"); //to control image preview style
...
<template>
  <div
    class="flex items-start py-2 px-8 border-b-[1px] border-b-slate-900/5 dark:border-b-gray-700"
  >
    <!-- avatar -->
    <div class="">
      <img
    ...
    <form class="flex flex-col w-full">
      ...
      <div
        ...
        :class="[gridWrapper,gridCols]"
      >
        <div class="relative" v-for="(url,index) in imgUrl" :key="index">
          <img
            :src="url"
            :class="['w-full object-cover rounded-2xl',imgRatio]"
            alt=""
          />
          <!-- dismiss button -->
          <div @click="removeImage(index)" class="absolute top-1 right-1 rounded-full p-2 bg-slate-900">
            <svg
              viewBox="0 0 24 24"
              aria-hidden="true"
              class="w-6 h-6 fill-white r-4qtqp9 r-yyyyoo r-dnmrzs r-bnwqim r-1plcrui r-lrvibr r-1hjwoze r-12ym1je"
