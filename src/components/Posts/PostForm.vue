...
const findHashtag = () => {
  setTimeout(() => {
    const regex = /#(\w+)/g;
    const match = form.content.match(regex);
    if (match) {
      console.log(match);
    }
  }, 300);
  ...
  >
    ...
    <form @submit.prevent="handleStoreposts" class="flex flex-col w-full">
      <textarea
        ...
        class="resize-none border-none bg-white focus:ring-white placeholder:text-lg text-lg dark:text-gray-300 no-scrollbar dark:bg-black dark:focus:ring-black"
