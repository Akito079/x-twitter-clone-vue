...
<template>
  ...
  >
    <div class="flex flex-col w-full gap-1 items-start">
      ...
      <div class="flex items-start gap-2 w-full">
        ...
        <div class="shrink-0">
          <img
            :src="'http://localhost:8000/profileImages/' + posts.userProfile"
      ...
      <div class="flex flex-col">
        ...
        <router-link
          :to="'/' + hashTag.name + '/' + hashTag.id"
          v-for="hashTag in posts.hashTags"
          :key="hashTag.id"
          class="text-blue-500"
          >#{{ hashTag.name }}</router-link
        >
      </div>
      <!-- content ends here -->
      <!-- images start here -->
          ...
          <img
            ...
            :class="[
              posts.media.length == 1
                ? ' w-full rounded-2xl max-h-[800px] object-cover'
      ...
      <div class="w-full">
        <Comment
          @reload-comments="() => (watchProp = !watchProp)"
          :post-id="posts.id"
          :user-id="user.id"
  ...
  <div v-else role="status" class="h-screen">
    <svg
      aria-hidden="true"
